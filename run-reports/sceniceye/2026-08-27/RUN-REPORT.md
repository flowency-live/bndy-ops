# ScenicEye run report — 2026-08-27

- **Run id:** `sceniceye-2026-08-27T12-39-59Z`
- **Outcome:** completed. Stale source week. No writes to bndy.
- **Runbook read:** `RUNBOOK.md` H1 **v2.27**. Floor in §6A is **v2.19**. The prompt states no number. Pass.
...
- **Source mode (§0.29):** the spec declares no mode. The run used **append-only**. It deleted nothing.
  The defect is already in `CTO-INBOX.md` as `sceniceye-mode-not-declared` (2026-08-12). It is not raised again.

## 1. Gates

| Step | Result |
|---|---|
| §6A.0 heartbeat | Written: `data/state/heartbeat/sceniceye-2026-08-27T12-39-59Z.json` |
| §6A.1 date | 2026-08-27 (Thursday), shell clock, 12:39:59Z |
| §6A.2a floor | v2.27 >= v2.19. Pass |
| §6A.2b claim | `data/state/claims/sceniceye.json` held `heldBy: null`, released by `sceniceye-2026-08-21T04-36-19Z`. Acquired. No takeover |
| §6A.3 tools | Chrome connected (1 browser, `7ad060c3-...`). bndy MCP reachable — `search_venue("The Heroes","Waterlooville")` returned `eb51991a-b082-433c-90e4-123340283271` |
...
- Hydration: read at 6 s, re-read at 15 s. The content was stable across both reads.
- Result: **7 day tables, 35 rows** (33 gig rows, 1 empty-day row, 1 day heading set).
- Raw capture: `data/raw/sceniceye/2026-08-27/capture.txt`.
- `javascript_tool` guards: output was returned in four pages and `=` was transformed, per §6B. No guard was hit.
...
- Today: **27 August 2026**.
- **Every one of the 35 rows is past-dated. Importable rows: 0.**

The curator rolls the page on a Thursday. The run fired at 12:39Z on Thursday 27 August, before the roll.
§0.14 forbids a past-dated import, and the spec's stale-week trap says to import nothing and stop cleanly.
This is a **clean stop, not a failure** (§6C `page-not-rolled`). It is not raised to `CTO-INBOX.md`.
...
|---|---|---|
| Capture vs stored snapshot (2026-08-21) | **0** | **0** |
...
| Venues created | 0 |
| Records enriched | 0 |
| Rows skipped as past-dated | 35 |
| Names sanitised or skipped as non-acts | 0 |
| Gate bounces (409/422) | 0 |
| Defaulted start times | 0 |
| Date corrections applied | 0 |

No row reached the venue, artist or event protocol, because the date filter rejected all 35 first.
...
It is recorded here so a future run does not rediscover it.

## 7. Snapshot

Written to `data/state/sceniceye-last-page.txt`. Content is byte-identical to the capture body.
...
`data/state/sceniceye-last-page.NEW.txt` remains a retired scratch file. It was not read and not written.

## 8. CTO-INBOX

Nothing was appended. A stale week is a clean stop, and the one defect this run met
